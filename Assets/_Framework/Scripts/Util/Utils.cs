...
namespace _Framework.Scripts.Util
{
    public static class Utils
    {
        ...
        public static float RangePercent(float value, float start, float end)
        {
        ...
        public static AudioSource PlayClipAtPoint3D(AudioClip clip, Vector3 pos, float volume = 1f, AudioMixerGroup audioMixerGroup = null, float pitch = 1, bool loop = false, float min = 1, float max = 500)
        {
        ...
        public static string AlternativeName(ISet<string> takenNames, string name)
        {
            takenNames = takenNames.Select(n => n.ToLower()).ToSet();
            
            while (takenNames.Contains(name.ToLower()))
            {
